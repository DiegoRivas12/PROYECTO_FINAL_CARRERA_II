...
def parse_args():
    ...
    parser.add_argument("--model_type",
                        type=str,
                        default="llama2",
                        choices=["alpaca", "open_llama", "llama2", "vicuna", "orca", "falcon", "baichuan_base","baichuan_chat", "stablebeluga","mistral", "platypus2"],
                        required=True)
...
def main():
    ...
    if args.model_type == "baichuan_base":
        ...
        print("baichuan_base model loaded")
        ...
        print("baichuan_chat model loaded")
    else:
        if args.model_type == "alpaca":
            tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path,fast_tokenizer=True, unk_token="<unk>",
                                                    bos_token="<s>",
                                                    eos_token="</s>")
        else:
    ...
    print('trainloader and evalloader loaded')
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = outputs.loss
            print_rank_0(f"Step {step+1}/{len(train_dataloader)}, Loss: {loss.item()}", args.global_rank)
            model.backward(loss)
            model.step()
